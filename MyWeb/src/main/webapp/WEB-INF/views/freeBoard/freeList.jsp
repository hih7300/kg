					...
					<table class="table table-bordered">
						...
						<c:forEach var="list" items="${list }">
							<tbody>
								<tr>
									<td>${list.bno }</td>
									<td><a href="freeDetail?bno=${list.bno }">${list.title }</a></td>
									<td>${list.writer }</td>
									<td><fmt:formatDate value="${list.regdate }"
											pattern="yy년MM월dd일 hh시mm분" /></td>
					...
					<form action="freeList" name="pageForm" id="pageForm">
						<div class="text-center">
							...
							<ul class="pagination pagination-sm">
								...
								<c:if test="${pageVO.prev }">
									<li>
										<a href="${pageVO.startPage-1 }" onclick="page(${pageVO.startPage-1 })">이전</a>
									</li>
								</c:if>
								<!-- 1. pageNum처리 -->
								<c:forEach var="num" begin="${pageVO.startPage }" end="${pageVO.endPage }">
...
<script>
	...
	window.onload = function() {
		...
		// 뒤로가기 실행후에 앞으로가기 했을때 변경된 기록이 공백이라면 함수 종료
		if (history.state == '') {
			return;
		}
	...
	function page(num) {
		...
		var pageNum = document.querySelector("#pageForm #pageNum").setAttribute("value", num); // 페이지폼의 값을 value로 세팅한다.
